...
exports.createClassroom = (req, res) => {
  ...
  classroom.save(async (err, classroom) => {
    ...
    const pushclassroom = await Promise.resolve(
      req.subject.classroom.push(classroom._id)
    );
    req.subject
      .save()
      ...
      .catch((err) => {
        ...
        Classroom.findByIdAndDelete(classroom._id)
          .then((data) => {
            return res
              .status(403)
              .json({ error: 'Cannot Add Classroom ' + err });
          })
          .catch((err) => {
...
exports.removeDocument = (req, res) => {
  var docs = [];
  Classroom.findById(req.body.cid).then((classroom) => {
